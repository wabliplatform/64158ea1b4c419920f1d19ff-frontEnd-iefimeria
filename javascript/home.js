let apiPApi = new TempApi.PApi();import TempApi from '../src/index';document.getElementById('i80j').onclick = (event) => {
    ...
    {   location.href= '/home' ;}};const onClickPaginationButtoniasxla = (chunk, pagination) => {
    ...
    let numberOfFrontButtons =  findTypeOfPaginationiasxla(pagination);
    pagination.children[chunk+numberOfFrontButtons-1].classList.add("active");

  apiPApi.getAllp((error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); const subDataElements =[...document.getElementById("iasxla").querySelectorAll( "[dataitem='true']" )].filter(
    (element, index, array) =>
    ...
    const findTypeOfPaginationiasxla = (pagination) => {
    ...
    const returnChunkIndexiasxla = (chunk, numberOfPages, cause) => {
  ...
  document.getElementById('i6jbh').onclick = (event) => {
    event.preventDefault();
    {  
  ...
  let pagination = document.getElementById("iasxla").querySelector('[pagination-list="true"]');
    ...
    for (let i = 0; i < numberOfPages; i++) {
      ...
      child.onclick = function () {
        if (chunk !== numberOfPages - i) {
          ...
          onClickPaginationButtoniasxla(chunk, pagination);
        }
      };
    ...
    for (let i = 0; i < pagination.children.length; i++) {
      ...
      if ( child.getAttribute("pagination-first") === "true" && numberOfPages > 0 ) {
        child.onclick = function () {
          if(chunk !== 1){
            chunk = 1;
            onClickPaginationButtoniasxla(1, pagination);
          }
        };
      }
      ...
      if ( child.getAttribute( "pagination-last" ) === "true" && numberOfPages > 0 ) {
        child.onclick = function () {
          if(chunk !== numberOfPages){
            chunk = numberOfPages;
            onClickPaginationButtoniasxla( numberOfPages, pagination);
          }
        };
      }
      ...
      if ( child.getAttribute("pagination-previous") === "true" && numberOfPages > 0 ) {
        child.onclick = function () {
          if(chunk !== returnChunkIndexiasxla(chunk,numberOfPages,'-')){
            chunk = returnChunkIndexiasxla(chunk,numberOfPages,'-');
            onClickPaginationButtoniasxla(chunk, pagination);
          }
        };
      }
